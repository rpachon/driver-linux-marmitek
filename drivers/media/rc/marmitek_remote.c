...
static const struct ati_receiver_type type_marmitek		= {
...
static void marmitek_remote_input_report(struct urb *urb)
{
	...
	if (urb->actual_length == 4) {
		...
		if (index >= 0 && marmitek_remote_tbl[index].kind == KIND_LITERAL) {
			input_event(dev, marmitek_remote_tbl[index].type,
				marmitek_remote_tbl[index].code,
				marmitek_remote_tbl[index].value);
			input_sync(dev);
		...
		if (index < 0 || marmitek_remote_tbl[index].kind == KIND_FILTERED) {
			unsigned long now = jiffies;
			...
			if (index < 0) {
				...
				while (count--) {
					...
					*/
					rc_keydown_notimeout(marmitek_remote->rdev, scancode,
							     data[2]);
			...
			input_event(dev, marmitek_remote_tbl[index].type,
				marmitek_remote_tbl[index].code, 1);
			input_sync(dev);
			input_event(dev, marmitek_remote_tbl[index].type,
				marmitek_remote_tbl[index].code, 0);
			input_sync(dev);
			 ...
			 */
			acc = marmitek_remote_compute_accel(marmitek_remote);
	
			switch (marmitek_remote_tbl[index].kind) {
			case KIND_ACCEL:
				input_event(dev, marmitek_remote_tbl[index].type,
					marmitek_remote_tbl[index].code,
					marmitek_remote_tbl[index].value * acc);
				break;
			case KIND_LU:
...
static void marmitek_remote_input_init(struct marmitek_remote *marmitek_remote)
{
	...
	for (i = 0; marmitek_remote_tbl[i].kind != KIND_END; i++)
		if (marmitek_remote_tbl[i].type == EV_KEY)
			set_bit(marmitek_remote_tbl[i].code, idev->keybit);
	...
	const struct usb_device_id *id)
{
 ...
 fail4:	rc_unregister_device(rc_dev);
	rc_dev = NULL;
 fail3:	usb_kill_urb(marmitek_remote->irq_urb);
	usb_kill_urb(marmitek_remote->out_urb);
 fail2:	marmitek_remote_free_buffers(marmitek_remote);
